...
float GestureRecognizer::getDistance(CCPoint pnt1, CCPoint pnt2) {
	float dx = pnt1.x - pnt2.x;
	float dy = pnt1.y - pnt2.y;
	return sqrt(dx * dx + dy * dy);
}

float GestureRecognizer::getInadicativeAngle(vector<CCPoint> points) {
	CCPoint centroid = getCentroid(points);
	return atan2(centroid.y - points[0].y, centroid.x - points[0].x);
}

CCPoint GestureRecognizer::getCentroid(vector<CCPoint> points) {
	float cx = 0;
	float cy = 0;
...
vector<CCPoint> GestureRecognizer::scalePointsTo(vector<CCPoint> points, float scaleSize) {
...
MatchingResult GestureRecognizer::doRecognize(vector<CCPoint> points, vector<MatchingTemplate> templates) {
	float b = INFINITY;
...
float GestureRecognizer::getDistAtBestAngle(vector<CCPoint> points, MatchingTemplate t, float theta1, float theta2, float theta3) {
...
float GestureRecognizer::getDistAtAngle(vector<CCPoint> points, MatchingTemplate t, float theta) {
...
MatchingResult GestureRecognizer::checkMatchingGesture(vector<CCPoint> points) {
...
void GestureRecognizer::initTemplates() {
	/** For test only */
...
void GestureRecognizer::generateIntoTemplate(string name, vector<CCPoint> points) {
	points = resamplePoints(points, RESAMPLE_POINT_COUNT);
	points = rotatePointsBy(points, -getInadicativeAngle(points));
	points = scalePointsTo(points, SCALE_SIZE);
	points = translatePointsTo(points, CCPoint(0, 0));
